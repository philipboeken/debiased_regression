#!/bin/bash
#
# Run as: run_experiment1.sh <n_iter=500> <n_obs=1000>
...
sbatch --dependency=afterok:$JOBID0 exp1d_plot_iterations_sbatch.sh 25 $NOBS npos indep
sbatch --dependency=afterok:$JOBID0 exp1d_plot_iterations_sbatch.sh 25 $NOBS pos dep
sbatch --dependency=afterok:$JOBID0 exp1d_plot_iterations_sbatch.sh 25 $NOBS npos dep

# For graph_nr=1, plot results of a single iteration for all pos_mode and indep_mode values
sbatch --dependency=afterok:$JOBID0 exp1e_compare_pos_indep.sh 1 1 $NOBS
